...
כל פסקה, כותרת, ציטוט וכו׳ נבדקים בנפרד (רשימות וטבלאות מטופלות כקבוצה - ראו שלב 6 ושלב 7). הזיהוי:
...
### שלב 7: תיקון רשימות (תבליטים ומספור)

<p dir="rtl">&#x200F;רשימת תבליטים (<code dir="ltr">&lt;ul&gt;</code>) או רשימה ממוספרת (<code dir="ltr">&lt;ol&gt;</code>) לא מטופלת פריט-פריט, אלא <strong>כיחידה אחת</strong>. הסיבה: כשרוב הרשימה עברית אבל פריט בודד מתחיל באנגלית (פקודה, שם שדה, נתיב, או פריט שכולו <code dir="ltr">code</code>), זיהוי per-פריט היה משאיר את אותו פריט מיושר לשמאל - והוא בלט החוצה משאר העמודה.</p>

<p dir="rtl">&#x200F;הכלל, ברמת הרשימה:</p>

<ul dir="rtl">
<li>אם <strong>לפחות פריט אחד</strong> ברשימה נוטה לעברית (מתחיל בעברית, או 30% ומעלה אותיות עבריות) ← <strong>כל</strong> הרשימה וכל פריטיה הופכים <code dir="ltr">direction: rtl</code> ומיושרים לימין, כולל הפריטים הלטיניים. כל פריט מקבל גם עוגן <strong>RLM</strong>&#x200F; וזיהוי חצים, כמו פסקה רגילה.</li>
<li>אם <strong>אף פריט</strong> ברשימה לא נוטה לעברית (רשימה שכולה אנגלית או קוד) ← הרשימה נשארת <code dir="ltr">direction: ltr</code> ומיושרת לשמאל. כפיית RTL על רשימה אנגלית רק פוגעת בקריאוּת.</li>
<li>זיהוי הפריט מתעלם מתוכן <code dir="ltr">code</code> (אותו <code dir="ltr">getText</code> כמו בפסקאות), כך שפריט שכולו <code dir="ltr">code</code> נחשב נייטרלי - לא מפעיל RTL בעצמו, אבל יורש את כיוון הרשימה אם פריט אחר עברי.</li>
<li>ה-MutationObserver מזהה רשימות חדשות ושינויי טקסט תוך כדי streaming - ברגע שפריט עברי נכנס לרשימה, כל הרשימה מתיישרת מחדש בזמן אמת.</li>
</ul>
